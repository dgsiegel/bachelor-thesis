...
class LocationPlugin(Plugin):
  ...
  def __init__(self):
    self.data = None
    self.out = []
    self.keys = ["address", "location", "lat", "long"]
  ...
  def output(self, page):
    page.div(class_="block_top block_location_top")
    page.div("Locations")
    page.div.close()
    ...
    page.div(class_="block_location_bottom")
      ...
      for v in self.out:
        ...
        for k in self.keys:
          for r in re.findall(k, v["text"], re.IGNORECASE):
            v["text"] = re.sub(r + " +(.+?)(\$|$)", r + " <span class=\"value\">\\1</span>", v["text"])
            v["text"] = re.sub(r, "<span class=\"keyword\">"+r+"</span>", v["text"])
            v["text"] = re.sub("\$", "", v["text"])

        page.div(class_="block_location_entry")
        page.div(v["text"], class_="block_location_entry_value")

        page.span("Posted on " + v["created_at"] + " using " + v["source"],
            class_="block_location_entry_value_info")
        if v["in_reply_to_status_id"] != None:
          page.span("In reply to status id: " +
              str(v["in_reply_to_status_id"]), class_="block_location_entry_value_info")
        if v["in_reply_to_screen_name"] != None:
          page.span("In reply to screen name: " +
              str(v["in_reply_to_screen_name"]), class_="block_location_entry_value_info")
